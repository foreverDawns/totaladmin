          ...
          <div class="order-list-info my-flex">
            <div class="order-list-one flex-h-center my-flex">
              <div class="order-img p-l-20 m-r-20">
                <img :src="item.orderGoodsPOS.length> 0 && item.orderGoodsPOS[0].image" alt="" srcset="">
              </div>
              <div class="order-title-box fw-700 flex-1">
                <div class="order-title fs-16 m-b-15">{{ item.orderGoodsPOS.length > 0 ? item.orderGoodsPOS[0].name : '' }}</div>
                <div class="order-title-size fs-14"> 规格：{{ item.orderGoodsPOS.length > 0 ? `${item.orderGoodsPOS[0].specList[0].key}/${item.orderGoodsPOS[0].specList[0].value}` : '' }}</div>
              </div>
              <div class="order-sell-num fs-16 fw-700">共{{ item.unm }}件</div>
            </div>
            <div class="order-list-two flex-h-center  fw-700 fs-16">￥{{ item.orderMoney }}</div>
            <div class="order-list-three flex-h-center fw-700 fs-16">{{ orderStatus[item.orderStatus] }}</div>
            <div class="order-list-four fw-700 fs-16">
              <div class="mb-10">{{ item.shippingUser}}</div>
              <div class="">{{ item.phoneNo }}</div>
            </div>
